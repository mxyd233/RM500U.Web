...
usage() {
    cat <<'EOF'
Usage: sudo ./install.sh [PUBLISH_DIRECTORY]
       sudo ./install.sh --bin-dir PUBLISH_DIRECTORY

PUBLISH_DIRECTORY defaults to ./publish relative to the script directory's
parent when the script is inside the project, or ./publish in the current
directory when it has been copied by itself. It must contain either
RM500U.Web (Native AOT/self-contained) or RM500U.Web.dll.

Options:
  -b, --bin-dir DIR  Use an already compiled/published application directory.
  -h, --help         Show this help.

The service unit is generated by this script, so install.sh can be copied
along with only the compiled output directory.
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        -h|--help)
            ...
            exit 0
            ;;
        -b|--bin-dir|--publish-directory)
            [ "$#" -ge 2 ] || { usage >&2; exit 2; }
            [ -z "$SOURCE_INPUT" ] || { usage >&2; exit 2; }
            SOURCE_INPUT=$2
            shift 2
            ;;
        --)
...
if [ -z "$SOURCE_INPUT" ]; then
    if [ -d "${PROJECT_DIR}/publish" ]; then
        SOURCE_INPUT="${PROJECT_DIR}/publish"
    else
        SOURCE_INPUT="$(pwd -P)/publish"
    fi
fi
...
if [ -f "$SERVICE_SOURCE" ]; then
    ...
    cat > "$UNIT_FILE" <<'EOF'
[Unit]
Description=RM500U Web Modem Console
...
User=root
Group=root
WorkingDirectory=/opt/rm500u-web

Environment=ASPNETCORE_ENVIRONMENT=Production
Environment=ASPNETCORE_URLS=http://0.0.0.0:5080
Environment=RM500U_DATA_DIR=/var/lib/rm500u-web
Environment=RM500U_SIMULATE=false
Environment=DOTNET_NOLOGO=true
Environment=DOTNET_CLI_TELEMETRY_OPTOUT=true
EnvironmentFile=-/etc/rm500u-web.env

ExecStart=/opt/rm500u-web/rm500u-web-launcher
Restart=on-failure
RestartSec=5
TimeoutStopSec=30
KillSignal=SIGINT
SyslogIdentifier=rm500u-web

StateDirectory=rm500u-web
